...
const MainPage = () => {
  ...
  const {isTonWalletConnected, friendlyAddress, tonBalance} = useCustomTonWallet();
...
const PHASES = [
  ...
  {
    question: "Phase 6",
    answer: "10 billion tokens sold * $0.000496/token = $4,960,000 (10% price increase.)",
  },
    ...
    if (paymenType == "ETH") {
      ...
      try {
        ...
        if(result.boc) {
          ...
          const response = await buyWithTon({data: hexString});
          if(response.ok) {
    ...
    <div>
      ...
      {error ? (
        ...
        <>
          ...
          {totalUsdRaised !== null ? (
            <p>Total USD Raised: ${totalUsdRaised}</p>
          ) : (
            <p>...</p>
          )}
        </>
      ...
      <HeroSection id="home">
        ...
        <div className="right !px-4 sm:!px-[2rem] pt-2 [clip-path:polygon(0%_1.5em,_1.5em_0%,_100%_0%,_100%_calc(100%_-_1.5em),_calc(100%_-_1.5em)_100%,_0_100%)]">
          ...
          <div className="rounded-lg w-full  mx-auto relative">
            {/* Payment Options */}
            <div className="flex justify-between h-[40px] space-x-2 sm:space-x-4  mb-4 relative cursor-pointer border-[2px] rounded-lg border-[#444444]">
              <div className="bg-transparent w-full" onClick={() => setIsPaymentDropdownOpen(!paymentDropdownOpen)}>
                <div className="flex justify-between items-center">
                  <button
                    className={`flex items-center sm:space-x-2 font-semibold text-white hover:bg- px-[.50rem] py-2 sm:px-4 sm:py-2 shadow-md `}
                  >
                    <img
                    ...
                    />
                    <span className="ml-2 text-[14px] sm:text-base">{paymenType}</span>
                  </button>
                  <FaAngleDown
                    color="white"
                    className={`w-5 h-5 mr-2 transition-transform duration-300 ease-in-out ${
            ...
            {paymentDropdownOpen &&
              <div className="payment-dropdown flex flex-col w-full absolute bg-[#212121] h-[200px] mt-[-14px] border-[2px] rounded-lg border-[#444444] z-20">
                <button
                  onClick={(e) => {
                    setIsPaymentDropdownOpen(false);
                    handlePaymentTypechange("ETH")
                  }}
                  className={`flex items-center sm:space-x-2 font-semibold hover:bg-custom-gradient-button text-white hover:bg- px-[.50rem] py-2 sm:px-4 sm:py-2 shadow-md`}
                >
                  <img
                    src="./ETH.svg"
                    alt="ETH"
                ...
                >
                  <img
                    ...
                    className="w-4 h-4 sm:w-5 sm:h-5"
                  />
                  <span className="text-[14px] sm:text-base">TON</span>
                </button>
              </div>
            }
            {paymenType === "TON" &&
              <div className="mb-2 [clip-path:polygon(0%_1em,_1em_0%,_100%_0%,_100%_calc(100%_-_1em),_calc(100%_-_1em)_100%,_0_100%)] relative bg-custom-gradient-button 
                ...
                <div className="ton [clip-path:polygon(0%_1em,_1em_0%,_100%_0%,_100%_calc(100%_-_1em),_calc(100%_-_1em)_100%,_0_100%)] absolute bg-white inset-[3px]">
                  ...
                  >
                    {isTonWalletConnected ? `Disconnect | ${friendlyAddress.substring(0, 4)}... | ${tonBalance} TON` : "Connect TON Wallet"}
                  </button>
                </div>
              </div>
            }
      ...
      <div className="bg-gradient-to-r from-[rgba(171,130,171,0.2)] via-[rgba(88,133,191,0.2)] to-[rgba(102,125,111,0.2)] text-white border-0 sm:clippath border-[#444444] mt-16 md:mx-20 rounded-lg shadow-md sm:p-8 ">
        ...
        <div className="flex flex-wrap gap-6 m-6 sm:px-8">
          <div className="md:w-[calc(50%-12px)] w-full flex flex-col gap-6">
            ...
            <div className="p-4 bg-gray-900 text-white clip">
              <h2 className="text-[1.125rem] sm:text-xl font-bold mb-2 bg-gradient-to-r from-custom-1 via-custom-2 to-custom-4 bg-clip-text text-white">
              Strategic Reserv And Airdrop: 5%
              </h2>
